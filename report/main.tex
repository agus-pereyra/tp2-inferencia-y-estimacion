...
\section{Desarrollo Experimental}
    ...
    \caption{HuffmanCoding}
        \begin{algorithmic}[1]
            \Require PMF de los símbolos $s_i$ de la cadena $S$ 
            \Ensure Códificación Hoffman para $S$
            \State $Q \gets$ lista de tuplas $(symbol, p_i | subtree)$
            \While{$Q$.size() > 1}
                \State Ordenar $Q$ por $p_i$
                \State $a, b \gets$ los 2 símbolos de $< p_i$
                \State Agregar 0 y 1 a los prefijos de $a$ y $b$ respectivamente
                \State Fusionar los subárboles
                \State create node $c$ with children $a$ and $b$ and weight $a.\text{weight} + b.\text{weight}$
                \State Q.push(c)
            \EndWhile
            \State \Return code obtained from the final tree
        \end{algorithmic}
    \end{algorithm}

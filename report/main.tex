...
\begin{multicols}{2}
    ...
    En un primer vistazo de este gráfico, vemos que el archivo \textit{adn.txt} muestra una distribucion completamente uniforme entre los cuatro simbolos que lo componen, es decir, $L = L_\text{unif}$ que se puede observar efectivamente más adelante. Cada uno aparece con una frecuencia alrededor del 25\% lo cual es coherente con la estructura de secuencia del ADN. Esto es un ejemplo sencillo para pensar la información que trasmite cada caracter. Partiendo de que, como definimos anteriormente, la información $\sim 1/p$ entonces cada caracter estaría representando $-\log_2(p)\approx 2$ bits de información. Si eliminamos un caracter de la cadena, estaríamos perdiendo demasiada información, y de hecho si hablamos de información genética, quitar un tipo de nucleótido dejaría a esta codificación genética biológicamente inservible.

    Lo último se ve reflejado en la compresión relativa en memoria que genera la codificación de Huffman para este texto, la cual es exactamente nula (observable en el Cuadro~\ref{tab:calcs}). La cadena no contiene redundancia alguna y por lo que el algoritmo no tiene oportunidad para aprovechar a ahorrar memoria, cada uno de los bits almacenados son indispensables.

    Algo fácil de relacionar entre el gráfico y la tabla es la entropía normalizada. A mayor cercanía al 1, significaría una distribución más uniforme en la frecuencia cada caracter y en nuestro caso con una precisión de 4 decimales sabemos que es casi perfectamente uniforme. En el resto de textos al tener un mayor $N$ y una concentración de ciertos caracteres, este valor es menor.
    ...
    Por otro lado, el archivo mitologia.txt muestra una distribucion de frecuencias muchos mas variadas. Notamos como unas pocas letras concentran una gran cantidad de las apariciones. Esto podria deberse a la estructura del lenguaje español. Asimismo, vemos como el caracter mas usado es el ``\_'' (espacio) lo que concuerda con la regla gramatical de separar las palabras con este simbolo. {\color{red}(Capaz puede decir algo mas)}
...
se obtiene una longitud promedio de 4.27 bits/simbolo y una reduccion de memoria de al 28.31\%. En este caso se puede apreciar claramente como la redundancia del lenguaje natural puede aprobecharse para comprimir informacion de manera eficiente.
